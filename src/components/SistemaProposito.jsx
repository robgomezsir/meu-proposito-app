...
const SistemaProposito = () => {
      ...
      return (
        <div className="min-h-screen flex items-center justify-center bg-gray-100">
          <div className="text-center p-8 bg-white rounded-2xl shadow-2xl max-w-md mx-4">
            ...
            <h1 className="text-3xl font-bold text-gray-800 mb-4">🚧 Sistema em Manutenção</h1>
            <p className="text-gray-600 mb-6">Estamos implementando melhorias importantes. Voltaremos em breve!</p>
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
              <p className="text-sm text-blue-800">
                <strong>🔄 Migração em Andamento:</strong> Firebase → Supabase
              </p>
            </div>
            <div className="text-xs text-gray-500">
              <p>⏰ Tempo estimado: Poucos minutos</p>
              <p>📧 Para acesso de emergência: robgomez.sir@gmail.com</p>
            </div>
          </div>
        </div>
      );
  ...
  useEffect(() => {
    ...
    if (savedUsuarios) {
      console.log('📱 Dados existentes no localStorage:', JSON.parse(savedUsuarios).length, 'usuários');
  ...
  // Função para buscar dados do Supabase (SOB DEMANDA)
  const handleBuscarDadosSupabase = async () => {
    console.log('🔘 BOTÃO CLICADO: handleBuscarDadosSupabase iniciado');
    ...
    try {
      ...
      console.log(`✅ ${usuariosSupabase.length} usuários carregados do Supabase`);
      
      // Atualizar estado local
      setUsuarios(usuariosSupabase);
      
      // Sincronizar com localStorage como backup
      localStorage.setItem('usuarios', JSON.stringify(usuariosSupabase));
      
      alert(`✅ Dados atualizados com sucesso!\n\n📊 Total de usuários: ${usuariosSupabase.length}\n💾 Dados sincronizados com localStorage`);
      
    } catch (error) {
      console.error('❌ Erro ao buscar dados do Supabase:', error);
      alert(`❌ Erro ao buscar dados:\n\n${error.message}\n\n📱 Dados do localStorage mantidos`);
    } finally {
          ...
          {!usuarioSelecionado ? (
            <div>
              ...
              <div className="mb-6 flex flex-wrap gap-4">
                ...
                <button
                  onClick={handleBuscarDadosSupabase}
                  className="flex items-center px-6 py-3 rounded-xl font-semibold bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white shadow-lg hover:shadow-xl transform hover:-translate-y-1 transition-all duration-200"
                >
                  <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
